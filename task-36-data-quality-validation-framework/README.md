# Task 36 — A repeatable data quality/validation framework

**Category:** Data Integrity & Quality

## Goal
Designing a repeatable validation/QC process around a load -- combining VALIDATION_MODE, post-load checks, and alerting into one pattern instead of ad hoc queries.

## Steps
1. Run `37_data_quality_validation_framework.sql`.
2. Work through it section by section against your own Snowflake trial
   — don't just read it.
3. Answer the understanding check below before moving to the next task.

## Understanding check
See the bottom of `37_data_quality_validation_framework.sql`. Answer by actually running the
diagnostic queries it points to, not from memory.

*(Status: scaffolded — SQL content not yet written.)*
